...
export default function Edit({ schools, professeurs, matieres, series, classe }) {
    ...
    // Pré-remplissage des lignes existantes depuis la relation classe.lignes
    const initialLignes = (classe?.lignes && classe.lignes.length > 0)
        ? classe.lignes.map((l) => ({
    ...
    const {
        ...
        serie_id: classe?.serie_id || classe?.serie?.id || '',
    ...
    const professeurOptions = professeurs.map((p) => ({
        value: p.id,
        label: `${p.firstname} ${p.lastname} ${authUser.user.id == 1 ? p.school?.raison_sociale ?? '' : ''}`,
    }));
