...
export default function Create({ schools, professeurs, matiere, professeurs_ids }) {
    ...
    const submit = (e) => {
        ...
        patch(route('matiere.update', matiere.id), {
            onSuccess: () => {
                Swal.close();
                Swal.fire({
                    icon: 'success',
        ...
        <AuthenticatedLayout
            header={
                <h2 className="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200 panel-title">
                    <CIcon className='text-success' icon={cibBuffer} /> Modification d'une matiere
                </h2>
            }
        >
                        ...
                        <div className="bg-light p-3 rounded border mb-5">
                            {checkPermission('matiere.view') ?
                                (<div className=" text-center  items-center gap-4">
                                    <Link className="btn btn-sm bg-success bg-hover text-white" href={route("matiere.index")}> <CIcon icon={cilList} /> Liste des matieres</Link>
                                </div>) : null
                            }
